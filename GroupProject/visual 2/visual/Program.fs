...
module Program=
    ...
    let RunVisualWithFlagsOut paras src =
        ...
        printfn "flagsint=%x, trace=%A" flagsInt trace.[5] // trace.[5] records the flag
        let flagBool n = (flagsInt &&& (1 <<< n)) > 0//check all four flags and returns whether true or false
        { 
          FN = flagBool 3
          FZ = flagBool 2
          FC = flagBool 1
    ...
    let VisualUnitTest name src (flagsExpected:string) (outExpected: (Out * int) list) =
        testCase name <| fun () ->
            ...
            /// returns [] if only Registers are used for memory operations
            /// otherwise, returns memory location
