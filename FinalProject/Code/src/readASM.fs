...
module readASM = 
        ...
        let isPureDigit (c:char) = List.contains c ['0'..'9']
        ...
        let(|AllInt|_|) (clst:char list) =
                ...
                |true -> None
(*
        let HexParse (clst:char list) = 
                clst |> List.rev |> ToIndexedList  |> List.sumBy ((fun (ind,c) -> match [c] with
                                                                                  |PosInt(digit) -> float digit*16.0**(float ind)
                                                                                  |['a'] -> 10.0*16.0**(float ind)
                                                                                  |['b'] -> 11.0*16.0**(float ind)
                                                                                  |['c'] -> 12.0*16.0**(float ind)
                                                                                  |['d'] -> 13.0*16.0**(float ind)
                                                                                  |['e'] -> 14.0*16.0**(float ind)
                                                                                  |['f'] -> 15.0*16.0**(float ind)
                                                                                  |_-> failwith "shouldn't happen") >> int)

        let BinParse (clst:char list) = 
                clst |> List.rev |> ToIndexedList |> List.sumBy ((fun (ind,c) -> match c with
                                                                                 |'0' -> 0.0
                                                                                 |'1' -> 2.0**(float ind)
                                                                                 |_->failwith "shouldn't happen") >> int)
        *)
        ...
        let (|GetInts|_|) strlst= 
                let isValid = List.forall (fun c -> let clst = c |> explode 
                                                    List.forall (fun c -> isPureDigit c || c = ',') clst) strlst

                let intParser (strlst:string list) = strlst
                                                     |> List.map (fun str -> match str |> explode with
                                                                             |PosInt(num) -> [num]
                                                                             |[','] -> []
...
//////////////////////////////////////////// ONE PATTERN PER INSTRUCTION /////////////////////////////////////////////// 
        ...
        let (|BLMatch|_|) (strlst: string list,labels:string list) =
                ...
                | _ -> None
(*
        let(|DCDMatch|_|) (strlst:string list) = 
                match strlst with
                |h::"dcd"::GetInts(intlst) when isValidLabel h -> DCD(h,0,intlst) |> Some
                |_-> None
        
        let (|EQUMatch|_|) (strlst:string list) =
                match strlst with
                |h::"equ"::GetInts([singint]) when isValidLabel h -> EQU(h,0,singint) |> Some
                |_-> None
*)
...
////////////////////////////////////////////////// MAIN CODE /////////////////////////////////////////////////////
        ...
        let getMachineState (strlst:string list) :Machinestate=
                ...
                let getlabel str = match str with
                                   |LabelMatch(label,clst) -> match label with 
                                                              |"abort" -> ( "" , ['@'])
                                                              | _ -> (label,clst)   
                                   | _ -> ("",str)
